...
RSpec.describe 'タスクモデル機能', type: :model do
  describe '検索機能' do
    ...
    context 'scopeメソッドでステータス検索をした場合' do
      it "ステータスに完全一致するタスクが絞り込まれる" do
        expect(Task.status_search('waiting')).to include(task)
        expect(Task.status_search('waiting')).not_to include(second_task)
        # ここに内容を記載する
      end
    end
  ...
  describe 'バリデーションのテスト' do
    context 'タスクのタイトルが空の場合' do
      it 'バリデーションにひっかる' do
        task = FactoryBot.build(:second_task)
    ...
    context 'タスクの詳細が空の場合' do
      it 'バリデーションにひっかかる' do
        # ここに内容を記載する
        task = FactoryBot.build(:third_task)
        expect(task).not_to be_valid
      end
    end
